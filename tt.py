...
def main():
    ...
    if status:
        ...
        print "=" * 40
        print `len(full_results)` + ' shortnames found'
        print
        print
...
def duplicates(args, enum_results):
    ...
    for result in enum_results:
        x = 2
        while True:
            status = GET(args, result[0].split('/')[-1] + '*~' + `x` + '*/.aspx')
            if status:
                print '\r\t' + result[0] + '~' + `x`
                duplicate_results.append((result[0], '*~' + `x` + '*/.aspx'))
                x = x + 1
            else:
                break
    print
    print '\r\t' + `len(duplicate_results)` + ' additional resources found by number cycling!'
    print
